...
### Шаг 4.3 — Кнопка «Играть» и команда /start

После деплоя API на Render бот настраивается **автоматически**, если заданы:

| Переменная | Значение |
|------------|----------|
| `BOT_TOKEN` | токен от BotFather |
| `FRONTEND_URL` или `WEB_APP_URL` | URL Vercel, например `https://ваш-проект.vercel.app` |

При старте сервера:
1. Внизу чата появляется **кнопка меню** «🏛️ Играть» (открывает Mini App)
2. На команду **`/start`** бот отвечает приветствием и **нижней клавиатурой** с кнопкой открытия игры

**Проверка вручную** (откройте в браузере после деплоя Render):

```
https://ВАШ-API.onrender.com/api/bot/setup
```

Должно быть: `{"ok":true,...}`

Затем в Telegram: напишите боту **`/start`**.

**Альтернатива через BotFather:** `/setmenubutton` → выберите бота → Web App → URL Vercel.
